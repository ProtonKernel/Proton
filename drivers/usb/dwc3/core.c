...
int dwc3_probe(struct platform_device *pdev, struct dwc3_exynos *exynos)
{
	...
	init_completion(&dwc->disconnect); // Added from the other branch
	...
	/* Disable LDO */
	exynos_usbdrd_phy_conn(dwc->usb2_generic_phy, 0); // Added from the other branch
...
err2:
	...
	pr_info("%s err = %d\n", __func__, ret); // Added from the other branch

	/* Disable LDO */
	//exynos_usbdrd_phy_conn(dwc->usb2_generic_phy, 0); // Commented out as it's redundant
